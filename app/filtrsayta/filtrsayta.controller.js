...
const MODEL_KEY = "filtrsayta"
const COLLECTION_NAME = "filtrsaytas"
...
// @desc    Get all filtrsayta
// @route   GET /api/filtrsayta
// @access  Private
export const getFiltrsaytas = asyncHandler(async (req, res) => {
...
// @desc    Get published filtrsayta (public)
// @route   GET /api/filtrsayta/public
// @access  Public
export const getPublicFiltrsaytas = asyncHandler(async (req, res) => {
...
// @desc    Get single filtrsayta
// @route   GET /api/filtrsayta/:id
// @access  Private
export const getFiltrsaytaById = asyncHandler(async (req, res) => {
  const model = getModel()
  const item = model
  ...
  if (!item) {
    res.status(404)
    throw new Error("Filtrsayta not found")
  }

  res.json(item)
})

// @desc    Create filtrsayta
// @route   POST /api/filtrsayta
// @access  Private
export const createFiltrsayta = asyncHandler(async (req, res) => {
  const model = getModel()
  const item = model
...
// @desc    Update filtrsayta
// @route   PUT /api/filtrsayta/:id
// @access  Private
export const updateFiltrsayta = asyncHandler(async (req, res) => {
  const model = getModel()
  ...
  res.json(updatedFiltrsayta)
})

// @desc    Delete filtrsayta
// @route   DELETE /api/filtrsayta/:id
// @access  Private
export const deleteFiltrsayta = asyncHandler(async (req, res) => {
  const model = getModel()
  const existing = model
    ? await model.findUnique({ where: { id: req.params.id } })
    : await findOneViaMongo(req.params.id)
  ...
  res.json({ message: "Filtrsayta deleted" })
})
